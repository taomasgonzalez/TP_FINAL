...
Package * PackageFactory::event_package_2_package(EventPackage* info_received)
{
	...
	case Event_type::ACK:
	{
		ACK_EventPackage * info_ack = static_cast<ACK_EventPackage *>(info_received);
		pac = new ACK_package(info_ack->give_me_your_package_ID());
	}
		break;
...
EventPackage * PackageFactory::package_2_event_package(Package * package_recieved)
{
	...
	case Package_type::MAP_IS: {
		...
		my_event_package = new MAP_IS_EventPackage(false, (char *)mi_pack->give_me_the_map(), mi_pack->give_me_the_checksum());
	}
	break;
	...
	default: //No puedo reconocer el paquete que llego, implica que est� corrompido, se produjo un error de comunicaci�n
		my_event_package = new ERROR_EventPackage(false);
		break;
	}
