...
LogicClientFSM::LogicClientFSM(Userdata * data, LogicEventGenerator *event_gen, Scene* scene, Communication* com): LogicFSM(data, event_gen, scene, com){
	...
	Waiting_for_map_state->push_back({ Event_type::RESET, this->Playing_state, reset_game_r });
	...
	Waiting_for_enemy_actions_state->push_back({ Event_type::RESET, this->Playing_state, reset_game_r });
	...
	Waiting_if_the_server_wants_to_play_again->push_back({ Event_type::RESET, this->Playing_state, reset_game_r });
...
void LogicClientFSM::print_curr_state()
{
	...
	else if (Waiting_for_servers_response_state == actual_state)
		cout << "Waiting_for_servers_response_state" << endl;
