...
void Map::load_on_map(const char* map_string, void* scenario) {
	...
	dijkstra_manager = new MapDijkstraMachine(number_of_rows, number_of_columns, map_string);
}
...
void Map::place_on_map_thing_vectors(MapThing* thing) {
	if (thing->is_proyectile())
		all_proyectiles->push_back((Proyectile*)thing);
	else if (thing->is_enemy())
...
Action_info Map::get_initial_enemy_actions() {
	static unsigned int loading_EA_number = 0;
