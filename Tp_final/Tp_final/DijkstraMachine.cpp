...
MapDijkstraMachine::MapDijkstraMachine(Map * map)
{
	...
	for (int i = 0; i < n_vertices; i++) {
		...
		if ((col + 1) < cols) 
			graph[i][i+1] = map->cell_has_floor(row, col + 1) ? 0 : 1;
		
		if ((col - 1) >= 0) 
			graph[i][i - 1] = map->cell_has_floor(row, col + 1) ? 0 : 1;
		
		if ((row + 1) < rows) 
			graph[i][(row+1)*cols + col] = map->cell_has_floor(row, col + 1) ? 0 : 1;
		
		if ((row - 1) >= 0) 
			graph[i][(row-1)*cols + col] = map->cell_has_floor(row-1, col) ? 0 : 1;
	}
