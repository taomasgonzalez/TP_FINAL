...
ProyectilesActionsFSMDRAWObserver::ProyectilesActionsFSMDRAWObserver(Proyectile * proyectile)
{
	...
	this->fsm = (ProyectilesActionsFSM*) proyectile->get_my_fsm();
	this->drawer = Userdata::drawer;
	this->ev_gen = (EventGenerator*) proyectile->get_my_ev_gen();
}
...
void ProyectilesActionsFSMDRAWObserver::update() {
	if (fsm->observer_info.move) {
		drawer->startDraw(proy_MOVING, proyectile->id, direction_translation(fsm->get_direction()), proyectile->pos_x, proyectile->pos_y);
		curr_state = proy_MOVING;
	}
	else if (fsm->observer_info.impact) {
		drawer->startDraw(proy_IMPACT, proyectile->id, direction_translation(fsm->get_direction()), proyectile->pos_x, proyectile->pos_y);
		curr_state = proy_IMPACT;
	}
	else if (fsm->observer_info.fall) {
		drawer->startDraw(proy_FALLING, proyectile->id, direction_translation(fsm->get_direction()), proyectile->pos_x, proyectile->pos_y);
