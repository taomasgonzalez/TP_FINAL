...
LogicClientFSM::LogicClientFSM(Userdata * data, LogicEventGenerator *event_gen, Scene* scene, Communication* com): LogicFSM(data, event_gen, scene, com){
	...
	Waiting_for_servers_response_state->push_back({ Event_type::MOVE, this->Playing_state, execute_receive_action_and_send_ack_r }); //extern MOVE that arrives through networking , has to be checked
	Waiting_for_servers_response_state->push_back({ Event_type::ATTACK, this->Playing_state, execute_receive_action_and_send_ack_r });  //extern ATTACK that arrives through networking , has to be checked
	Waiting_for_servers_response_state->push_back({ Event_type::LOCAL_QUIT, this->Waiting_for_ACK_quit_state, send_quit_r }); //se recibe un envio un quit local, paso a esperar el ACK
	Waiting_for_servers_response_state->push_back({ Event_type::EXTERN_QUIT, NULL, send_ack_and_quit_r }); //se recibe un quit por networking,
	Waiting_for_servers_response_state->push_back({ Event_type::ACTION_REQUEST, this->Waiting_for_servers_response_state, do_nothing_r });
	Waiting_for_servers_response_state->push_back({ Event_type::ERROR1, NULL, analayze_error_r });
	Waiting_for_servers_response_state->push_back({ Event_type::END_OF_TABLE, this->Waiting_for_servers_response_state, do_nothing_r });
