...
void Scene::load_new_map(bool is_client, const char * the_map, unsigned char the_checksum ) {
	...
	if (is_client) //The map came by networking
	{
		//if (actual_map == -1)
		//	actual_map++;
		new_map->load_on_map((const unsigned char*)the_map, this);
	...
	{	//I´m server, I´ve the map available
		...
		new_map->load_on_map((const unsigned char *)map_string.c_str(),this);
		new_map->load_checksum(make_checksum((const unsigned char*)map_string.c_str()));
